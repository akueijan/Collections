    ...
    methods: {
        ...
        checkBrower(page) {
            ...
            if(window.innerWidth > 1000) {
                if(navigator.userAgent.match('Instagram') || navigator.userAgent.match('FB')) {
                    vm.isPc = false
                } else {
                    vm.isPc = true
                    document.querySelector("body").style = "overflow: hidden"
...
class slickUse {
    ...
    videoPlay() {
        var cun;
        cun = Math.floor($(this.bd + " video").length / 2);
        $(this.bd).on('beforeChange', function(event, slick, currentSlide, nextSlide){
